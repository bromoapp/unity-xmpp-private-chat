...
namespace Xmpp.Net
{
    ...
    public class ClientSocket : BaseSocket
    {
        ...
        {
            base.Connect();
            Debug.Write(this.GetType().ToString() + ">>> TRY TO OPEN SOCKET");
            ...
            try
            {
                ...
                Debug.Write(this.GetType().ToString() + ">>> IPADDRESS: " + ipAddress.ToString());
            ...
            {
                Debug.Write(this.GetType().ToString() + "ERROR\n" + ex.ToString());
                base.FireOnError(ex);
            }
        }
        ...
        {
            Debug.Write(this.GetType().ToString() + ">>> CONNECTING TIMEOUT");
            connectTimeoutTimer.Dispose();
        ...
        {
            Debug.Write(this.GetType().ToString() + ">>> SENDING\n" + data);
        ...
        private void EndReceive(IAsyncResult ar)
        {
            try
            {
                ...
                if (nBytes > 0)
                {
                    Debug.Write(this.GetType().ToString() + ">>> RECEIVED\n" + System.Text.Encoding.UTF8.GetString(m_ReadBuffer, 0, nBytes));
